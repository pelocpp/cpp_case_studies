...
template <typename T>
class PermutationCalculatorExEx
{
private:
    PermutationExEx<T> m_p;
    PermutationContainer<T> m_perms;

public:
    // c'tor
    PermutationCalculatorExEx() : m_p{}, m_perms{} {}

    // getter/setter
    void setPermutation(const PermutationExEx<T>& p) {
        m_p = p;
    }

    PermutationExEx<T> getPermutation() {
        return m_p;
    }

    // public interface
    void calculate() {
        m_perms = calculate(m_p);
    }

    PermutationContainer<T> calculate(const PermutationExEx<T>& p) {
        if (p.grade() == 1)
        {
        ...
        else
        {
            ...
            for (size_t i{}; i != p.grade(); ++i)
            {
                ...
                PermutationExEx<T> q{ p.removeAt(i) };
    ...
    //PermutationContainer<T> calculateEx(const PermutationExEx<T>& p) {
