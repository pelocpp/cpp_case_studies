...
void MandelbrotBase::computeRects(int rows, int cols)
{
    ...
    std::size_t clientWidth{ m_clientWidth - (m_clientWidth % cols) };
    std::size_t clientHeight{ m_clientHeight - (m_clientHeight % rows) };

    // calculate size of a single rectangle
    std::size_t rectWidth{ clientWidth / cols };
    std::size_t rectHeight{ clientHeight / rows };
...
std::pair<std::wstring, std::size_t> MandelbrotBase::paintRectangle(HDC hDC) const
{
