...
std::list<Token> InfixToPostfixConverter::convert(const std::list<Token> infix)
{
    ...
    // TODO: m_tok ist irhendwie ein schlechter Namen

    while ((m_tok = nextToken()).getTokenType() != TokenType::Null)
    {
        if (m_tok.getTokenType() == TokenType::Operand)
            processOperand();
        else if (m_tok.getTokenType() == TokenType::Operator)
            processOperator();
        else if (m_tok.getTokenType() == TokenType::LBracket)
            processLeftParenthesis();
        else if (m_tok.getTokenType() == TokenType::RBracket)
...
void InfixToPostfixConverter::processOperator()
{
    ...
    do
    {
        ...
        if (m_tok.getPrecedence() <= top.getPrecedence())
        {
            m_stack.pop();
